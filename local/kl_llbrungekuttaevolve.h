 ...
 *
 * KL(m) BEGIN
 * This is an implementation of the Landau-Lifshitz-Bloch dynamical
 ...
 * Thermal noise is not included in this implementation.
 *
 * Important usage note.
 * The class Klm_LLB_RKEvolve must be used together with thermal correction
 * to the interactions, via usage of class:
 *   Klm_LLB_Term
 * Additionally, if you include interactions (a usual case), you should compute them without
 * the assumption of |m|=1. Thus, taking the class Oxs_UniformExchange as an example,
 * you should specify kernel parameter value:
 *   kernel "NGBR_6_LLB"
 * BEFORE using any interaction you should make sure that it is compatible with LLB.
 *
 * Programming principle
 *   In standard LLG-approach you have time-variable m-vector that has unit-length
 *   (plus numerical deviations from the rule |m|==1). This vector has to be
 *   multiplied by the time-constant ("read-only") Ms-scalar. 
 *   In LLB-approach the time-variable m-vector is numerically alwas unit-long.
 *   Changes in its length are stored in the Ms-scalar - that is a time-variable
 *   now ("read-write"; change #1). 
 *   Additionally there is a time-constant 
 *   Ms_T0-scalar that represents the magnetization equlibruium length 
 *   for the zero temperature ("read-only"; change #2). Additionally there is another
 *   read-only scalar, Me_T, stored in the simstate, but not used in the RK procedure.
 *   Last change #3 is maybe 
 *   obvious, although a littlpe bit complex to code: the LLG calculation (for dm_dt) 
 *   got replaced by the LLB calculation (for dM_dt, of course)--first of all the
 *   parallel_damping term was added there, a term resposible for the changes |M(t)|.
 *   Site note. 
 *   As a result: the |m|!=1 normalization error, norm_error, is always 0.0 now.
 *   This should not be a problem, as citing Mike (mail from 30.1.2019):
*       If you search for
*       norm_error in that routine, you'll see that although each of the stepper
*       routines computes norm_error the value is never used.  So setting it to
*       0.0 should be fine.
*
 * KL(m). Note KLnote1
 ...
 *   norm_error             0.0, see above
 *   start_dm               start_dM
 *   mxH                    <unchanged>
 *   <nothing>              H, LLB needs more information, so we have to pass this
 *                             variable as well
 *   error_estimate (1)     error_estimate (A/m)    // change of units
 *
 ...
 * Kristof Lebecki, 2010-2019, Konstanz-Warsaw
...
private:
  ...
  // KL(m), monitor of strange cases
  // Warning fired when |M(i)|/Ms_T0-1 > expected_m_max_precision
  OC_REAL8m expected_m_max_precision; // Cases |M|>|Ms_T0|
  // Warning fired when |M(i)|/Ms_T0-1 < expected_m_min
  ...
  // KL(m), note: mdt_style is set to DMDT_STANDARD in the constructor
  ...
  //
  // KL(m)
  // Table used as Calculate_dM_dt-input parameter 
  // holding the H values.
  // This input parameter is calculated by (earlier) 
  // GetEnergyDensity function.
  // Note: we need multiple vtmp-variable because they store /both/ mxH and
  // dM_dt. In vtmpH we store only-H, so one array is enough.
  Oxs_MeshValue<ThreeVector> vtmpH;
  ...
  void AdjustState(OC_REAL8m hstep,
		   ...
		   const Oxs_MeshValue<ThreeVector>& dM_dt, // KL(m)
  ...
  /// Returns maximum difference between vectors multiplied by factors, in corresponding
...
#define RKStepFuncSig(NAME)                              \
  ...
  template <typename DMDT> void Calculate_dM_dt // See cc-file KLnote1
  ...
  /// Imports: state_, mxH_, H_, pE_pt
  /// Exports: dM_dt_, max_dM_dt_, dE_dt_, min_timestep_

  // Member function pointer to one of the templated Calculate_dM_dt
  // member functions
  void (Klm_LLB_RKEvolve::* calculate_dM_dt_ptr) // KL(m)
  (const Oxs_SimState&,const Oxs_MeshValue<ThreeVector>&,
   const Oxs_MeshValue<ThreeVector>&, // KL(m). Additionally needed by LLB
   OC_REAL8m,
  ...
  void Compute_dM_dt_core_and_Advance(const Oxs_SimState* workstate, // KL(m)
                                      DMDT& dMdt,                    // KL(m)
                                      ADVANCE& advance)
  {
    Compute_dM_dt_core_and_Advance_helper<DMDT,ADVANCE>              // KL(m)
      (workstate,dMdt,advance);                                      // KL(m) and so on below...
  }
  ...
  class Standard_dMdt {
  public:
    Standard_dMdt(const Klm_LLB_RKEvolve* rke,
                  const Oxs_SimState& work_state,
                  const Oxs_MeshValue<ThreeVector>& import_mxH,
                  const Oxs_MeshValue<ThreeVector>& import_H,    // KL(m) additionally needed for LLB
      ...
      :  mesh(work_state.mesh),
         ...
         reltemp13(1.-(rke->relative_temperature/3.)), // KL(m)
         reltemp23(rke->relative_temperature*(2./3.)), // KL(m)  
         H(import_H),    			       // KL(m)  
         Ms_T0(*(work_state.GetPtr_Ms_T0()))	       // KL(m)
    {}
    ...
    inline void ComputeCore(OC_INDEX j) {
      ...
      if(Ms[j]==0) {
        ...
        ThreeVector scratch_precess
          = (do_precess ? Ms[j]*mxH[j] : ThreeVector(0,0,0)); // KL(m), precessional term updated
        ThreeVector scratch_damp = spin[j];
        scratch_damp ^= mxH[j]; // (mx(mxH))
              
        const OC_REAL8m mH = spin[j]*H[j];
        scratch_precess.Accum(alpha[j]*reltemp13*Ms_T0[j],scratch_damp);   // KL(m), adding perpend. damping term
        scratch_precess.Accum(-alpha[j]*reltemp23*Ms_T0[j]*mH,spin[j]); // KL(m), adding parallel damping term
        // KL(m) all three terms: precessional + perp_damping + parallel_damping 
    ...
    inline void Compute(OC_INDEX j,
                        ...
                        OC_REAL8m& pE_pM_sum) { // KL(m), note: here "M" was present form the beginning
      if(Ms[j]==0) {
        dM_dt[j].Set(0.0,0.0,0.0);
      } else {
        // Note: mxH may be same as dm_dt
        const ThreeVector torque = mxH[j];
        // KL(m) old LLG versus new LLB formulas:
        ...
        // LLB_parallel_damping part  
        //      V.H.dM/dt = V.{ |gamma|alpha(1-t/3)MS_T0(mxH)^2 + m.H(d|M|/dt) }
        // (as dE_dt_sum will be later multiplied by -MU0)
        //
        ThreeVector scratch_precess
          = (do_precess ? Ms[j]*mxH[j] : ThreeVector(0,0,0)); // KL(m), precessional term updated
        OC_REAL8m dM_dt_sq = (do_precess ? Ms[j]*Ms[j] : 0.0);// KL(m), precessional term (dM_dt_sq) updated

        const OC_REAL8m a = alpha[j];
        dM_dt_sq += a*a*reltemp13*Ms_T0[j]*reltemp13*Ms_T0[j];// KL(m), perp_damping term (dM_dt_sq) updated
              
        const OC_REAL8m mxH_sq = torque.MagSq();
        const OC_REAL8m g = gamma[j];
        dM_dt_sq *= mxH_sq * g * g;
        // KL(m), dM_dt_sq with all pre-factors: precessional_term + perp_damping_term
              
        const OC_REAL8m mH    = spin[j]*H[j];              // KL(m) 
        const OC_REAL8m mH_sq = mH*mH;                     // KL(m) 
        dM_dt_sq += g*g*a*a*reltemp23*reltemp23*Ms_T0[j]*Ms_T0[j]*mH_sq; // KL(m)
        // KL(m), full dM_dt_sq: precessional_term + perp_damping_term + parallel_damping

        if(dM_dt_sq>max_dM_dt_sq) {
          max_dM_dt_sq = dM_dt_sq;
        }
        // KL(m) note. Attention: minus in front.
        pE_pM_sum -=  mesh->Volume(j) * a * g * Ms_T0[j] * // KL(m) note, here: Ms_T0[j] instead of Ms[j]
                      ( reltemp13 * mxH_sq                 // KL(m) perp_damping term
                       +reltemp23 *  mH_sq )  ;            // KL(m) parallel_damping term

        ThreeVector scratch_damp = spin[j];
        scratch_damp ^= torque; // (mx(mxH))
        scratch_precess.Accum(a*reltemp13*Ms_T0[j],scratch_damp); // KL(m), adding perpend. damping term
        // mxH + alpha(1-t/3)MS_T0(mx(mxH))
        // KL(m) precessional+perp_damp term updated

        scratch_precess.Accum(-1*a*reltemp23*Ms_T0[j]*mH,spin[j]);  // KL(m), adding parallel damping term
        // mxH + alpha(1-t/3)MS_T0(mx(mxH)) - alpha(2t/3)MS_T0.m.(m.H) 
        // KL(m) all three terms: precessional + perp_damping + parallel_damping 
        // Minus/negative comment: usually gamma (aka "g") is negative.
        // Thus, for example, parallel_damping term in above computations has
        // a minus sign, although in the LLB equations it is positive.
  ...
  private:
    ...
    const OC_REAL8m reltemp13;             // KL(m)
    const OC_REAL8m reltemp23;             // KL(m)
    const Oxs_MeshValue<ThreeVector>& H;   // KL(m). Additionally needed by LLB
    const Oxs_MeshValue<OC_REAL8m>& Ms_T0; // KL(m). Additionally needed by LLB
...
//  public:
//    Baryakhtar_dmdt(const Klm_LLB_RKEvolve* rke,
//                  const Oxs_SimState& work_state,
//                  const Oxs_MeshValue<ThreeVector>& import_mxH,
...
//  class Zhang_dmdt {
//  public:
//    Zhang_dmdt(const Klm_LLB_RKEvolve* rke,
//               const Oxs_SimState& work_state,
//               const Oxs_MeshValue<ThreeVector>& import_mxH,
//               Oxs_MeshValue<ThreeVector>& export_dm_dt)
//      // NOTE: import_mxH and import_dm_dt may be same array.
