...
// to wipe "spin" too.
{
  ...
  Ms_T0_ptr=NULL;
  Me_T_ptr =NULL;
  // Previously: setting to NULL. Now, we "touch" it only in non-ownership case.
  // Otherwise it is left *untouched*.
  if(!Ms.IsOwner())
    Ms.SetAsNonOwner(NULL);
  if(!Ms_inverse.IsOwner())
    Ms_inverse.SetAsNonOwner(NULL);
  // KL(m) end	
  max_absMs=-1;
...
void Oxs_SimState::CloneHeader(Oxs_SimState& new_state) const
{
  ...
  // new_state has just been created by the constructor.
  /// We set its Ms pointers and adjust their size - we do
  /// *not* copy anything here
  if( Ms.IsOwner() == TRUE && new_state.Ms.IsOwner() == FALSE && 
      new_state.Ms.GetPtr() == NULL ) {
    // No copying, just ajust the mesh
    const Oxs_MeshValue<OC_REAL8m>* Ms_new_ptr;     // Pointers
    const Oxs_MeshValue<OC_REAL8m>* Ms_new_inv_ptr;
    // New state has no owned pointers. We have first to allocate them.
    ...
    // Finally we can use created pointers
    new_state.Ms.SetAsOwner( Ms_new_ptr );
    new_state.Ms_inverse.SetAsOwner( Ms_new_inv_ptr );
  }
  // Make sure new_state has same ownership as this.
  /// I am not sure about necessity of this check, I just want
  /// to track it.
  /// We can implement here changing ownership old.owner->new.owner
  /// as well. It would require some memory managment, however.
  /// Like, destroying or constructing a pointer.
...
Oxs_SimState& Oxs_SimState::operator=(const Oxs_SimState &org) {
  ...
  // eventually copying large tables
  if( !org.Ms.IsOwner() ) {
    // Copy pointers
    ...
    if( Ms.IsOwner() && Ms_inverse.IsOwner() ) {
      // New states have already owned pointers. We have only to copy.
      Ms_new_ptr     = Ms.GetPtr();
      Ms_new_inv_ptr = Ms_inverse.GetPtr();
      // This would copy just pointers:
      //      Ms_new_ptr = org.Ms.GetPtr();
      // But here we copy pointer contents
      // We need non-const cating for that
    ...
    else
    {
      ...
      Ms_new_ptr     = new const Oxs_MeshValue<OC_REAL8m>; // Allocate
      Ms_new_inv_ptr = new const Oxs_MeshValue<OC_REAL8m>;
      // We need here non-const pointer; We perform a copy here
      *(const_cast<Oxs_MeshValue<OC_REAL8m>*>(Ms_new_ptr)) 
                                    = *(org.Ms.GetPtr());
      *(const_cast<Oxs_MeshValue<OC_REAL8m>*>(Ms_new_inv_ptr))
                                    = *(org.Ms_inverse.GetPtr());
...
{ // Fill state from channel
  ...
  if(Ms.IsOwner() || Ms_inverse.IsOwner()) // KL(m)? Is this line necessary?
  {
    ...
    // Old-style, no ownership, copy pointer
...
// KL(m)
  ...
  if(mesh==NULL || Ms.GetPtr()==NULL || Ms_inverse.GetPtr()==NULL
  || import_Ms==NULL || import_Ms_inverse==NULL) {
// KL(m) end  
