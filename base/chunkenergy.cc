...
public:
  ...
  Oxs_MeshValue<ThreeVector>* H;       // KL(m)
  Oxs_MeshValue<ThreeVector>* H_accum; // KL(m)
  ...
  while(1) {
    ...
    for(OC_INDEX icache_start=index_start;
      ...
      while(i_fixed < i_fixed_total) {
        ...
        if(H)         (*H)[index].Set(0.,0.,0.);       // KL(m) 
        if(H_accum)   (*H_accum)[index].Set(0.,0.,0.); // KL(m)
        ++i_fixed;
      }
...
#define FUDGE 8
  ...
  chunk_thread.H         = oced.H;       // KL(m)
  chunk_thread.H_accum   = oced.H_accum; // KL(m)
